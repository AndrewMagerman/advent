...
def next_position(pos: Tuple, direction: str) -> Tuple:
    k = {'R': lambda x, y: (x + 1, y),
         'L': lambda x, y: (x - 1, y),
         'U': lambda x, y: (x, y + 1),
         'D': lambda x, y: (x, y - 1),
         }

    return k[direction](pos[0], pos[1])
